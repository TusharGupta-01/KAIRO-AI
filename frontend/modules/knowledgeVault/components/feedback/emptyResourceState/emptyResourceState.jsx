...
const EmptyState = ({ icon: Icon, title, description, actions = [] }) => {
  return (
    <div className="flex flex-col items-center rounded-3xl border border-dashed border-zinc-800 bg-zinc-900 px-8 py-20 text-center">
      ...
      <div className="mt-8 space-y-2">
        ...
        <p className="mx-auto max-w-md leading-7 text-zinc-400">
          {description}
        </p>
      </div>
      ...
      {actions.length > 0 && (
        <div className="mt-8 flex flex-wrap justify-center gap-3">
          {actions.map(
            ({ label, icon: ActionIcon, variant = "primary", onClick }) => (
              <button
                key={label}
                type="button"
                onClick={onClick}
